...
function extractMetadata(html: string, url: string) {
  ...
  if (articleAuthorMatch && !metadata.author) metadata.author = articleAuthorMatch[1].trim();
  ...
  const datePubMatch = html.match(/<meta[^>]*name=["']date["'][^>]*content=["']([^"']+)["']/i);
  if (datePubMatch && !metadata.publishDate) metadata.publishDate = datePubMatch[1].trim();
...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    const { url, useAI } = JSON.parse(event.body || "{}");
    ...
    if (useAI && GEMINI_API_KEY && (!metadata.author || !metadata.publishDate)) {
...
{"author": "Author Name or Organization", "publishDate": "YYYY-MM-DD or n.d.", "improvedTitle": "Cleaned up title if needed"}`;
      ...
      try {
        ...
        if (aiResponse.ok) {
          ...
          if (jsonMatch) {
            ...
            if (aiResult.author && aiResult.author !== "Unknown") metadata.author = aiResult.author;
            if (aiResult.publishDate && aiResult.publishDate !== "n.d." && !metadata.publishDate) {
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        message: useAI ? "Extracted with AI enhancement" : "Extracted from page metadata",
      }),
    };
  } catch (error) {
